...
BENCHMARKS = ["Godunov", "TimeStep", "EulerSimulation", "ConsToPrim", "PrimToCons"]
...
# Plot
# ---------------------------------------------------------
def plot_all(files, speedups, out_dir):
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    datasets = {label: load(path) for label, path in files.items()}
    ...
    for bm in BENCHMARKS:
        ...
        ax1.grid(True)
        plt.tight_layout()
        path = out_dir / f"{bm}.png"
        plt.savefig(path, dpi=200)
        plt.close()
        print("saved:", path)


plot_all(FILES, SPEEDUPS, OUT_DIR)
