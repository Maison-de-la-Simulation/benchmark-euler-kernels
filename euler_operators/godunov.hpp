        ...
        T const dt)
{
    ...
    IndexType const stride_1 = prim_arrays.d.stride(1);
    IndexType const stride_2 = prim_arrays.d.stride(2);
    EulerPrimArrays const prim_ptrs = data_handle(prim_arrays);
    EulerConsArrays const cons_ptrs = data_handle(cons_arrays);

    auto f = KOKKOS_LAMBDA(IndexType const i, IndexType const j, IndexType const k)
    {
    ...
    Kokkos::Array<IndexType, 3> const begin {1, 1, 1};
    Kokkos::Array<IndexType, 3> const
            end {1 + (prim_arrays.d.extent(0) - 2),
                 1 + (prim_arrays.d.extent(1) - 2),
                 1 + (prim_arrays.d.extent(2) - 2)};
    euler_parallel_for("godunov", exec_space, begin, end, f);
}
