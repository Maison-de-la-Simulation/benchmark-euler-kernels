...
void GodunovVectorizedUnrolled(benchmark::State& state)
{
    auto const n = int_cast<index_t>(state.range() + 2);
    PerfectGas<real_t> const eos(1.4);
    UniformMesh3d<real_t> const mesh(1., 1., 1.);
    ...
    for ([[maybe_unused]] auto _ : state) {
        godunov_vec_unrolled(exec_space, as_const(prim_arrays), cons_arrays, eos, mesh, hllc(), dt);
        exec_space.fence();
        benchmark::ClobberMemory();
    }
...
BENCHMARK(GodunovVectorized2)
        ->UseRealTime()
        ->DenseRange(8, 31, 8)
        ->DenseRange(32, 127, 16)
        ->DenseRange(128, 320, 32);

BENCHMARK(GodunovVectorizedUnrolled)
        ->UseRealTime()
        ->DenseRange(8, 31, 8)
        ->DenseRange(32, 127, 16)
        ->DenseRange(128, 320, 32);
