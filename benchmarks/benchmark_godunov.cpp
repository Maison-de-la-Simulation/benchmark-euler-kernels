...
void GodunovHllcOpti(benchmark::State& state)
{
    auto const n = int_cast<index_t>(state.range());
    std::size_t const ng_z = n + 2;
    index_t const ng = n + 2;
    ...
    for ([[maybe_unused]] auto _ : state) {
        godunov(exec_space, as_const(prim_arrays), cons_arrays, eos, mesh, hllc_opti(), dt);
        exec_space.fence();
        benchmark::ClobberMemory();
    }
...
void GodunovOptiHllcOpti(benchmark::State& state)
{
    auto const n = int_cast<index_t>(state.range());
    std::size_t const ng_z = n + 2;
    index_t const ng = n + 2;
    ...
    for ([[maybe_unused]] auto _ : state) {
        godunov_opti(exec_space, as_const(prim_arrays), cons_arrays, eos, mesh, hllc_opti(), dt);
        exec_space.fence();
        benchmark::ClobberMemory();
    }
...
void GodunovVectorizedHllcOpti(benchmark::State& state)
{
    auto const n = int_cast<index_t>(state.range() + 2);
    PerfectGas<real_t> const eos(1.4);
    UniformMesh3d<real_t> const mesh(1., 1., 1.);
    ...
    for ([[maybe_unused]] auto _ : state) {
        godunov_vec(exec_space, as_const(prim_arrays), cons_arrays, eos, mesh, hllc_opti(), dt);
        exec_space.fence();
        benchmark::ClobberMemory();
    }
...
BENCHMARK(GodunovHllcOpti)
        ->UseRealTime()
        ->DenseRange(8, 31, 8)
        ->DenseRange(32, 127, 16)
        ->DenseRange(128, 320, 32);

BENCHMARK(GodunovOptiHllcOpti)
        ->UseRealTime()
        ->DenseRange(8, 31, 8)
        ->DenseRange(32, 127, 16)
        ->DenseRange(128, 320, 32);

BENCHMARK(GodunovVectorizedHllcOpti)
        ->UseRealTime()
        ->DenseRange(8, 31, 8)
        ->DenseRange(32, 127, 16)
        ->DenseRange(128, 320, 32);
